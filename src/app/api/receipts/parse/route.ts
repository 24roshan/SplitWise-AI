...
export async function POST(req: NextRequest) {
  ...
  const rl = checkRateLimit(`receipt-parse:${session.userId}`, { max: 15, windowMs: 60_000 });
  if (!rl.allowed) {
  ...
  if (!process.env.OPENAI_API_KEY) {
    return NextResponse.json(
      { error: "AI receipt parsing isn't configured on this server. Enter items manually." },
      { status: 503 }
    );
  }
  ...
  try {
      ...
      messages: [
        {
          role: "user",
          content: [
            { type: "text", text: "Extract the line items and total from this receipt." },
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Could not read that receipt clearly. Try a clearer photo or enter items manually." },
        { status: 422 }
      );
    }
